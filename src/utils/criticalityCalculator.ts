...
interface ContentFactors {
  contentType: 'finance' | 'health' | 'entertainment' | 'education' | 'government' | 'commerce' | 'media' | 'social';
...
export function calculateCriticality(
  ...
  const totalScore = performanceScore + businessScore + geoScore + redundancyScore + networkScore;
...
function calculatePerformanceScore(metrics: NetworkMetrics): number {
  ...
  const latencyScore = metrics.latency < 50 ? 10 : 
                      metrics.latency < 100 ? 8 :
                      metrics.latency < 200 ? 5 : 2;
  
  // Bandwidth (0-10): >1000Mbps=10, 500-1000=8, 100-500=5, <100=2
  const bandwidthScore = metrics.bandwidth > 1000 ? 10 :
                        metrics.bandwidth > 500 ? 8 :
                        metrics.bandwidth > 100 ? 5 : 2;
  
  // Reliability (0-10): >99%=10, 95-99%=8, 90-95%=5, <90%=2
  const reliabilityScore = metrics.reliability > 99 ? 10 :
                          metrics.reliability > 95 ? 8 :
                          metrics.reliability > 90 ? 5 : 2;
  
  return latencyScore + bandwidthScore + reliabilityScore;
}

function calculateBusinessImpact(content: ContentFactors, trafficVolume: number): number {
  ...
  const volumeScore = trafficVolume > 1000 ? 10 :
                     trafficVolume > 500 ? 8 :
                     trafficVolume > 100 ? 5 : 2;
...
function calculateGeopoliticalScore(geo: GeopoliticalFactors): number {
  ...
  ) * 2; // Multiplicar por 2 para escala 0-20
}

function calculateRedundancyScore(redundancy: number, cdnType: string): number {
  // Menos redundância = mais crítico (score INVERSO)
  const baseScore = redundancy === 0 ? 15 :    // Único ponto de falha
                   redundancy === 1 ? 12 :     // Pouca redundância
                   redundancy <= 3 ? 8 :       // Alguma redundância
                   redundancy <= 5 ? 5 : 2;    // Muita redundância
  
  // CDNs locais são mais críticos se únicos
  const cdnMultiplier = cdnType === 'local' ? 1.2 :
                       cdnType === 'regional' ? 1.1 : 1.0;
...
function calculateNetworkQuality(metrics: NetworkMetrics): number {
  ...
  const hopScore = metrics.hopCount <= 3 ? 5 :
                  metrics.hopCount <= 6 ? 3 :
                  metrics.hopCount <= 10 ? 2 : 1;
